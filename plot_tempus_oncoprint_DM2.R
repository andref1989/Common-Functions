...
#'
#' @param cohort Path to patient cohort of interest or the named list object output by load_tempus_data. The path option is slower and not advised if multiple oncoprints will be generated sequentially.
...
#' }
#'
plot_tempus_oncoprint_v2 <- function(cohort, genes_of_interest=NULL,alteration_func = NULL, patient_identifier = "analysis_id", gene_identifier = c("gene_canonical_name","gene_symbol"), title = NULL, filter_germline = TRUE, copy_number_threshold = 8, patient_order = NULL,patient_universe=NULL,assay_blacklist=NULL) {
...
##########################################################################################
#### Formatting mmf files for oncoprint if DM1
##########################################################################################
    if(data_model_check){
...
##########################################################################################
#### Formatting mmf for oncoprint if DM2
##########################################################################################
    

        if(!data_model_check){

            mmf <- input_td[grep("onco_result_cnv|onco_result_snv", names(input_td))]
            gene_identifier <- unique(unlist(lapply(mmf, function(x) intersect(gene_identifier,colnames(x)))))

            mmf_SNV <- input_td[grep("snv", names(mmf))]
            mmf_CNV <- input_td[grep("cnv", names(mmf))]
      ...
      mmf_SNV <- dplyr::filter(mmf_SNV, .data$variant_classification != "B",!grepl("stream|intron|UTR",variant_molecular_consequence)) %>%
          dplyr::select(.data$patient_id, .data$analysis_id, .data$Gene, functional_impact=.data$variant_classification,mutation_effect=.data$variant_molecular_consequence, variant_allele_freq= .data$tumor_variant_allele_frequency, somatic_germline=dplyr::any_of(c("variant_original_source","variant_origin")))
  } else {
    mmf_SNV <- dplyr::filter(mmf_SNV, .data$variant_classification != "B",!grepl("stream|intron|UTR",variant_molecular_consequence)) %>%
      dplyr::select(.data$patient_id, .data$analysis_id, !!as.name(patient_identifier), .data$Gene, functional_impact=.data$variant_classification, mutation_effect=.data$variant_molecular_consequence, variant_allele_freq=.data$tumor_variant_allele_frequency, somatic_germline=dplyr::any_of(c("variant_original_source","variant_origin")))
  }
  if (filter_germline) {
    mmf_SNV <- dplyr::filter(mmf_SNV, .data$somatic_germline %in% c("S","somatic"))
      ...
      if (patient_identifier %in% c("patient_id", "analysis_id")) {
    mmf_CNV_ref <- dplyr::select(mmf_CNV,.data$patient_id, .data$analysis_id, .data$Gene, .data$copy_number)
  } else {
    mmf_CNV_ref <- dplyr::select(mmf_CNV,.data$patient_id, .data$analysis_id, !!as.name(patient_identifier), .data$Gene, .data$copy_number)
  }
...
##########################################################################################
        ...
        mmf_final <- cbind(mmf_final, missing_mat)
            ...
            if(!data_model_check){
            patient_universe <- unique(unlist(lapply(mmf, function(x) x[,patient_identifier])))} else{ patient_universe <- unique(mmf[,patient_identifier])}
            missing_patients <- setdiff(colnames(mmf_final),x= patient_universe)
            missing_mat <- matrix("", nrow=nrow(mmf_final), ncol=length(missing_patients))
            colnames(missing_mat) <- missing_patients
            rownames(missing_mat) <- rownames(mmf_final)
