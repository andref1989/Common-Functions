...
#' @param genes_of_interest Vector of genes the user wants plotted (MANDATORY if
...
#' @param filter_germline This is a boolean for whether germline mutations should
#' be removed from consideration. If you've removed the somatic_germline column
#' from the mmf, this should be changed to FALSE
#' @param alteration_func A named list object that assigns color and sizes to
#' different mutation types (Optional)
#' @param copy_number_threshold The minimum copy number required to call an increase
#'  in copy number an amplification. Anything below this value will be classified
#'  as a "weak amplification" (integer)
#' @param patient_order The ordering of patients during final plotting. This is
#' helpful if you plan to add additional annotations later. (vector)
#' @param patient_universe The total set of patients in the universe (only necessary
#'  if using a filtered version of the mmf file as input where some patients may not
#'  have alterations. (Optional vector)
#' @param assay_blacklist The specific assays you want to exclude from the oncoprint
#' (Optional string/vector)
#' @param minimum_copy_number_threshold The minimum copy number to be considered a weak amplification, allows for fine tuning thresholds for plotting
#' @param variant_blacklist The variants you want to exclude from consideration in your plot. Default is exclude any that are not pathogenic or likely pathogenic (US,CE,B,LB).
#' @param drop_LOH A toggle to drop heterozygous copy number loss from the heatmap. Default FALSE
...
#' }
#'
plot_tempus_oncoprint <- function(data_cohort,
                                  ...
                                  minimum_copy_number_threshold=3,
                                  drop_LOH=FALSE) {
...
############  Define DM specific functions ##################
    prep_oncoprint_dm1 <- function(input_td){
      ...
      mmf_SNV <- dplyr::filter(mmf, .data$variant_type_code == "SHRTVRNT", !.data$functional_impact  %in% variant_blacklist,!grepl("stream|UTR",.data$mutation_effect)) %>%
...
##########################################################################################
    ...
    if(drop_LOH){
        mmf_int$Mut <- gsub("SHALLOWDEL","",mmf_int$Mut)}
  mmf_out <- mmf_int %>%
...
##########################################################################################
        ...
        mmf_final <- cbind(mmf_final, missing_mat)
            ...
            mmf_locs <- input_td[grep(paste0(c("g_molecular_master_file","g_molecular_master_file_filtered","onco_result_cnv_chr_segment","onco_result_cnv_gene"),collapse="|"),names(input_td))]
  ...
  if (is.null(alteration_func)) {
    ...
    if(drop_LOH){
      ...
      OTHER = ComplexHeatmap::alter_graphic("rect", height = 0.2,width=0.2, fill = col[["OTHER"]])
)} else{
    alteration_func <- list(
      background = ComplexHeatmap::alter_graphic("rect", fill = "#CCCCCC"),
      HOMDEL = ComplexHeatmap::alter_graphic("rect", fill = col[["HOMDEL"]]),
    ...
    mmf_final[is.na(mmf_final)] <- ""
       saveRDS(mmf_final,"~/MMF_final2.rds") 
    ht_legend_param <- list(title = "Alterations", at = names(col), labels = c("Deep Deletion", "Shallow Deletion", paste0("Amplification (>=", copy_number_threshold, ")"), paste0("Weak Amplification (",minimum_copy_number_threshold,"-", copy_number_threshold - 1, ")"), "Missense Mutation", "Truncating Mutation","Splice Region","Other"))
    ...
    print("Custom color palettes hasn't been implemented yet")
    ## return(ht)
  } else {
